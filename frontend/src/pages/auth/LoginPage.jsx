...
export default function LoginPage() {
  const [form, setForm] = useState({ email: "owner@stockflow.demo", password: "password123" });
    ...
    <main className="auth-page">
      <section className="auth-visual-panel">
        ...
        <div className="auth-visual-content">
          ...
          <p>Continue managing stock, sales, invoices, customer credit and staff activity.</p>
          <div className="auth-testimonial">
            <strong>Demo account</strong>
            <p>The login fields are already completed. Select “Log in” to open the working dashboard.</p>
          </div>
        </div>
      </section>
      ...
      <section className="auth-form-panel">
        <div className="auth-form-card">
          ...
          <form onSubmit={handleSubmit} className="auth-form">
            ...
            <Button type="submit" size="large" className="full-width-button" disabled={submitting}>
              {submitting ? "Logging in..." : "Log in"}
            </Button>
          </form>

          <p className="auth-switch-text">New to StockFlow? <Link to="/register">Create an account</Link></p>
        </div>
      </section>
    </main>
  );
